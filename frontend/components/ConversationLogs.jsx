...
import { MonitorIcon, DownloadIcon, ClipboardIcon, SearchIcon, EyeIcon, CalendarIcon, ClockIcon, ChatIcon, CheckIcon, BugIcon, ExternalLinkIcon } from './icons/Icons.jsx';
...
Generated by Purl Agent System - purl.cat
  ...
  /** Share conversation to X (Twitter) */
  const shareToX = (conversation) => {
    const baseUrl = window.location.origin;
    const conversationUrl = `${baseUrl}/logs?id=${conversation.conversationId}`;
    
    // Try to get an interesting agent response from the conversation
    let tweetText = "Just discovered this chat log with $purl";
    
    // Get messages from memories (preferred) or fallback to conversation messages
    const memMessages = (allMemories || [])
      .filter(m => (m.roomId || m.channelId) === conversation.conversationId)
      .sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
    
    const messages = memMessages.length > 0 ? memMessages : (conversation.messages || []);
    
    // Find a good agent response to include in the tweet
    const agentResponses = messages.filter(m => 
      m.userId === 'agent' && 
      m.content && 
      m.content.text && 
      m.content.text.length > 20 && 
      m.content.text.length < 150 &&
      !m.content.text.startsWith('I') && // Avoid first-person responses
      !m.content.text.toLowerCase().includes('sorry') &&
      !m.content.text.toLowerCase().includes('help')
    );
    
    if (agentResponses.length > 0) {
      // Pick a random interesting response
      const randomResponse = agentResponses[Math.floor(Math.random() * agentResponses.length)];
      let responseText = randomResponse.content.text;
      ...
      tweetText = `Just discovered this chat log with $purl: "${responseText}"`;
    }
    
    const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(tweetText)}&url=${encodeURIComponent(conversationUrl)}`;
      ...
      <div className={`conversation-viewer ${theme}`}>
        ...
        <div className="viewer-controls">
          ...
          <div className="actions-section">
            ...
            <button onClick={() => shareToX(selectedConversation)} className="share-x-button" title="Share to X (Twitter)">
              <ExternalLinkIcon />&nbsp;Share to X
            </button>
            ...
            {paginatedItems.map((conversation) => (
              <div key={conversation.conversationId} className="conversation-card" onClick={() => viewConversation(conversation)}>
                ...
                <div className="card-actions">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); shareToX(conversation); }} className="action-button share-x" title="Share to X (Twitter)"><ExternalLinkIcon /></button>
                </div>
              </div>
            ))}
          </div>
